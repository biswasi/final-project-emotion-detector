...
def sent_detector():
    input_to_detect = request.args.get('textToAnalyze')
    response = emotion_detector(input_to_detect)
    formated_response = emotion_predictor(response)

    if formated_response['dominant_emotion'] is None:
        return "Invalid text! Please try again."
    return (
        f"Given statement, the system response is 'anger': {formated_response['anger']} "
